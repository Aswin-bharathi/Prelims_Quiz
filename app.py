...
def init_db():
    try:
        ...
        for student in c.fetchall():
            ...
            c.execute('UPDATE students SET password = %s WHERE id = %s', (password, student_id))
...
def add_team():
    if request.method == 'POST':
        ...
        try:
            ...
            conn = mysql.connector.connect(**db_config)
            c = conn.cursor()
            c.execute('INSERT INTO students (lotname, password, quiz_status_tech, quiz_status_software) VALUES (%s, %s, %s, %s)',
...
def update_team(id):
    ...
    if request.method == 'POST':
        ...
        try:
            password = lotname + '@2k25'
            c.execute('UPDATE students SET lotname = %s, password = %s WHERE id = %s',
                     (lotname, password, id))
            conn.commit()
...
def show_teams():
    ...
    if search:
        query += ' WHERE lotname LIKE %s'
        count_query += ' WHERE lotname LIKE %s'
        params.append(f'%{search}%')
    
    if quiz_type != 'all':
        status_column = 'quiz_status_tech' if quiz_type == 'Tech' else 'quiz_status_software'
        if search:
            query += f' AND {status_column} != %s'
            count_query += f' AND {status_column} != %s'
        else:
            query += f' WHERE {status_column} != %s'
            count_query += f' WHERE {status_column} != %s'
    ...
    c.execute(query + ' LIMIT %s OFFSET %s', params + [per_page, (page-1)*per_page])
    teams = c.fetchall()
    conn.close()
...
def add_question():
    if request.method == 'POST':
        ...
        conn = mysql.connector.connect(**db_config)
        c = conn.cursor()
        c.execute('INSERT INTO questions (quiz_type, question, option1, option2, option3, option4, answer) VALUES (%s, %s, %s, %s, %s, %s, %s)',
...
def delete_question(id):
    conn = mysql.connector.connect(**db_config)
    c = conn.cursor()
    c.execute('DELETE FROM questions WHERE id = %s', (id,))
    conn.commit()
    conn.close()
...
def set_entry_code():
    if request.method == 'POST':
        ...
        conn = mysql.connector.connect(**db_config)
        c = conn.cursor()
        c.execute('INSERT INTO quiz_entries (entry_code, quiz_type) VALUES (%s, %s)', (entry_code, quiz_type))
        conn.commit()
        conn.close()
...
def view_results():
    ...
    if quiz_type != 'all':
        query += ' WHERE results.quiz_type = %s'
        count_query += ' WHERE results.quiz_type = %s'
        params.append(quiz_type)
    
    if search:
        condition = ' AND ' if quiz_type != 'all' else ' WHERE '
        query += f'{condition}(students.lotname LIKE %s)'
        count_query += f'{condition}(students.lotname LIKE %s)'
        params.append(f'%{search}%')
    ...
    top_teams_params = []
    if quiz_type != 'all':
        top_teams_query += ' WHERE results.quiz_type = %s'
...
def quiz_login():
    if request.method == 'POST':
        ...
        c.execute('SELECT quiz_type FROM quiz_entries WHERE entry_code = %s', (entry_code,))
        entry = c.fetchone()
        ...
        c.execute(f'SELECT * FROM students WHERE lotname = %s AND password = %s AND {quiz_status_column} = %s',
...
def quiz_start():
    ...
    conn = mysql.connector.connect(**db_config)
    c = conn.cursor()
    c.execute('SELECT question, option1, option2, option3, option4, answer FROM questions WHERE quiz_type = %s', (session['team']['quiz_type'],))
    questions = c.fetchall()
    conn.close()
...
def quiz_submit():
    ...
    conn = mysql.connector.connect(**db_config)
    c = conn.cursor()
    c.execute('INSERT INTO results (lotname, score, duration, quiz_type) VALUES (%s, %s, %s, %s)',
             (session['team']['lotname'], score, duration, quiz_type))
    c.execute(f'UPDATE students SET quiz_status_{quiz_type.lower()} = %s WHERE lotname = %s',
